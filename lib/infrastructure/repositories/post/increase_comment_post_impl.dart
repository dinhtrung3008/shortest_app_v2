...
@LazySingleton(as: IIncreaseCommentsPostCount)
class IncreaseCommentsPostCountImpl
    with ExecuteRepositoryImpl, ConnectionChecker
    implements IIncreaseCommentsPostCount {
  final IPostRemoteService _iPostRemoteService;

  IncreaseCommentsPostCountImpl(this._iPostRemoteService);
  ...
  Future<Either<BaseFailure, PostShortest>> call({required String postId}) async {
    ...
    return execute<PostShortest>(
      action: () async {
        final response = await _iPostRemoteService.increaseComments(postId: postId);
