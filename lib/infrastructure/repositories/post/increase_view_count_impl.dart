...
@LazySingleton(as: IIncreaseViewsPostCount)
class IncreaseViewsPostCountImpl with ExecuteRepositoryImpl, ConnectionChecker implements IIncreaseViewsPostCount {
  final IPostRemoteService _iPostRemoteService;

  IncreaseViewsPostCountImpl(this._iPostRemoteService);
  ...
  Future<Either<BaseFailure, PostShortest>> call({required String postId}) async {
    ...
    return executeRepositoryService<PostShortest>(
      action: () async {
        final response = await _iPostRemoteService.increaseViewsCount(postId: postId);
